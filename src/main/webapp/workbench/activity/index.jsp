...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#editBtn").click(function () {
			...
			// var id=$($("input[name=xz]:checked")[0]).val();;
			// $.ajax({
		...
		$("#updateBtn").click(function () {
			...
			if(confirm("是否修改活动信息？")){
				...
				var editOwner=				$("#edit-marketActivityOwner").val();

				var editActivityName=		$("#edit-marketActivityName").val();

				var editStartDate=			$("#edit-startTime").val();

				var editEndDate=			$("#edit-endTime").val();

				var editCost=				$("#edit-cost").val();

				var editdescription=		$("#edit-describe").val();

				var editBy="${user.name}";
		...
		$("#searchBtn").click(function () {
			// var searchName=$.trim(		$("#search-name").val());
			// var searchOwner=$.trim(		$("#search-owner").val());
			// var searchStartDate=$.trim(	$("#search-startTime").val());
			// var searchEndDate=$.trim(	$("#search-endTime").val());
		...
		$("#deleteBtn").click(function () {
			...
			if($xz.length==0){
				...
				if(confirm("确定删除选中记录？")){
					...
					//将$xz中的每一个dom对象遍历出来,取其value值，就是要删除的活动的id
	...
	function pageList(pageNo,pageSize) {
		...
		$("#qx").prop("checked",false);
		//alert("展示市场活动列表");
		var searchName=$("#hidden-name").val();
		var searchOwner=$("#hidden-owner").val();
		var searchStartDate=$("#hidden-startDate").val();
		var searchEndDate=$("#hidden-endDate").val();

		$("#search-name").val(searchName);
		$("#search-owner").val(searchOwner);
		$("#search-startTime").val(searchStartDate);
		$("#search-endTime").val(searchEndDate);
		...
		$.ajax({
			...
			success:function (data) {
				/*
				data包含：
				需要的市场信息活动列表 List<Activity> aList，总的记录数
				{"total":100,"dataList":[市场活动1，市场活动2，...]}



				 */
				//alert("数据总数："+data.total);
				var html="";
